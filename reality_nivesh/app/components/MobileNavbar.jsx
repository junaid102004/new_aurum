...
const MobileNavbar = ({ isOpen, setIsOpen }) => {
    ...
    return (
        ...
        transform transition-transform duration-700 ease-in-out ${isOpen ? "translate-x-0" : "translate-x-full"
                    }`}
            >
                {/* Header */}
                        ...
                        <ul className="flex flex-col space-y-2 text-lg font-medium text-white">
                            {menuItems.map((item) => (
                                ...
                                >
                                    ...
                                    {/* Submenu (expandable) */}
                                    <div
                                        className={`transition-all duration-500 overflow-hidden ${expandedItems[item.id] ? "max-h-96 mt-2" : "max-h-0"
                                            }`}
                                    >
                                        {item.subItems && (
